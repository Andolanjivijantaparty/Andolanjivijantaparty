...
export default function ContactForm() {
    ...
    const validate = () => {
        ...
        if (form.name.trim().length < 3) {
            er.name = 'कृपया अपना नाम लिखें।';
        }

        if (!/^\S+@\S+\.\S+$/.test(form.email.trim())) {
            er.email = 'कृपया सही ईमेल लिखें।';
        }

        if (form.message.trim().length < 5) {
            er.message = 'कृपया अपना संदेश लिखें।';
        }
    ...
    return (
        <form
            onSubmit={onSubmit}
            noValidate
            className="space-y-5 rounded-lg border border-border bg-card p-6 shadow-sm sm:p-8"
        >
